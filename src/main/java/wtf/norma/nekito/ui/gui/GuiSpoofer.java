...
import wtf.norma.nekito.helper.player.PlayerUuidHelper;
...
public class GuiSpoofer extends GuiScreen {
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        switch (button.id) {
            ...
            case 4: {
                if (this.field_3.getText().trim().isEmpty()) {
                    this.field_2.setText(PlayerUuidHelper.getUUID(mc.getSession().username) != null ? PlayerUuidHelper.getUUID(mc.getSession().username).toString() : "Error");
                    break;
                }
                this.field_2.setText(PlayerUuidHelper.getUUID(this.field_3.getText().trim()) != null ? PlayerUuidHelper.getUUID(this.field_3.getText().trim()).toString() : "Error");
                break;
            }
            case 5: {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        GuiSpoofer.drawCenteredString(mc.fontRendererObj, "Fake IP", width / 2, height / 2 - 20 - 10, Color.WHITE.hashCode());
        GuiSpoofer.drawCenteredString(mc.fontRendererObj, "Cracked Username", width / 2, height / 2 - 20 - 45, Color.WHITE.hashCode());
        GuiSpoofer.drawCenteredString(mc.fontRendererObj, "Fake UUID", width / 2, height / 2 - 20 + 25, Color.WHITE.hashCode());
        GL11.glPushMatrix();
        GL11.glColor4d(1.0, 1.0, 1.0, 1.0);
        GL11.glScaled(4.0, 4.0, 4.0);
        GuiSpoofer.drawCenteredString(mc.fontRendererObj, renderText, width / 8, height / 4 - mc.fontRendererObj.FONT_HEIGHT, 0);
        GL11.glPopMatrix();
